  ...
  mStandalone(false)
{
  ...
  } else
  {
    ...
    if (node.isProcessingInstruction())
    {
      ...
      while (p > -1)
      {
        if (regExp.capturedTexts().count()==2 && regExp.cap(1)=="version")
          mXmlVersion = regExp.cap(2);
        else if (regExp.capturedTexts().count()==2 && regExp.cap(1)=="encoding")
          mEncoding = regExp.cap(2);
        else if (regExp.capturedTexts().count()==2 && regExp.cap(1)=="standalone")
